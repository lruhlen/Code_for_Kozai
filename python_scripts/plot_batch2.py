...
#============================================================
def plot_batch2(data): # Note: 'data' has to be a dataframe type (as in, what search_index.py loads into the selected_data variable)
    ...
    msize = 6
    semi_val = round(data.SEMI[0]/AU,3)
    ecc_val = data.ECCN[0]
    titlestr="Semi-major axis: "+str(semi_val)+" AU, Eccentricity: "+str(ecc_val)
    plt.figure(1)        
    ...
    for i in range(len(files)):
        ...
        if not os.path.exists(item+"/atmos_data.txt"):
            ...
            plt_label = '~'+str(round(masses[i]/mJup,3)) + ' Mjup'
